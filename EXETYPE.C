...
enum exeKind __stdcall exeType(char *f)
{
	...
	errno = 0;
	if ( (fh = _sopen( f, _O_RDONLY | _O_BINARY, _SH_DENYWR )) == -1 )
	    return IOERROR;
	...
	if ( br != sizeof oldhdr || E_MAGIC(oldhdr) != EMAGIC )
	    ...
	    {
		_lseeki64( fh, E_LFANEW(oldhdr), SEEK_SET);
        ...
        if ( br != sizeof newhdr ) retc = OLDEXE;
		     ...
		     {
			_lseeki64( fh, NEDEFSTUBMSG, SEEK_SET );
...
char * __stdcall strExeType (enum exeKind exenum)
{
    switch ( exenum )
    {
