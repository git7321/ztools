...
forfile (
    char *pat,
    int attr,
    void (__stdcall *rtn)(char *, struct findType *, void *),
    void * args
    )
{
    ...
    if ((fbuf = (struct findType *)(char *)malloc(sizeof (struct findType))) == NULL)
	    return FALSE;
    ...
    if ((buf = (char *)malloc(_MAX_DRIVE + _MAX_DIR + _MAX_FNAME + _MAX_EXT + 1)) == NULL) {
	    findclose (fbuf);
	    free ((char *) fbuf);
	    return FALSE;
	}
